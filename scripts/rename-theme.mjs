...
import { replacements, TEMPLATE_PACKAGE_NAME } from "./theme-identity.config.mjs";
...
function createIdentity(input) {
  ...
  if (name.length > 80 || !/^[A-Za-z]+(?: [A-Za-z]+)*$/.test(name)) {
    throw new Error("Use 1–80 Latin letters and single spaces, for example: Awesome Theme.");
  }
...
async function main() {
  if (await isRenamed()) {
    process.stdout.write("Theme is already renamed.\n");
    return;
  }
  ...
  try {
    ...
    process.stdout.write(`\nTheme renamed to ${identity.name}.\n`);
  } finally {
    terminal.close();
  }
}
